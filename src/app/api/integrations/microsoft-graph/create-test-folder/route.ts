...
import { createTestJobFolder } from "@/lib/integrations/microsoft-graph-test-folder";

/**
 * Temporary owner-only test: create "GRAPH TEST - DELETE ME" in Jobs folder.
 * POST /api/integrations/microsoft-graph/create-test-folder
 */
export async function POST() {
  ...
  try {
    const folder = await createTestJobFolder();
    return NextResponse.json({
      ok: true,
      message: "Test folder created inside MICROSOFT_JOBS_FOLDER_ID.",
      folder: {
        name: folder.name,
        id: folder.id,
