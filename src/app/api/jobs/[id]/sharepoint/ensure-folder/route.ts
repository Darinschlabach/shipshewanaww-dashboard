...
import { ensureJobSharePointFolders } from "@/lib/integrations/microsoft-graph-job-folders";

type RouteContext = {
  params: Promise<{ id: string }>;
};

/** POST /api/jobs/[id]/sharepoint/ensure-folder */
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    const folder = await ensureJobSharePointFolders(jobId);
    ...
    return NextResponse.json(
      {
        ...
        error:
          err instanceof Error
            ? err.message
            : "Could not ensure SharePoint job folder.",
      },
      { status: 500 }
    );
  }
