...
import { renameJobSharePointFolder } from "@/lib/integrations/microsoft-graph-job-folders";
...
/**
 * POST /api/jobs/[id]/sharepoint/rename-folder
 * Body: { jobName: string }
 */
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const folder = await renameJobSharePointFolder({
      jobId,
      newJobName: jobName,
    });
    ...
    return NextResponse.json(
      {
        ...
        error:
          err instanceof Error
            ? err.message
            : "Could not rename SharePoint job folder.",
      },
      { status: 500 }
    );
  }
