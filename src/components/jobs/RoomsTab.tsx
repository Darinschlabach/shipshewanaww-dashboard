            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <CreatableSpecSelect
                ...
                onRenameOption={async (oldName, newName) => {
                  const saved = await renameDoorStyleOption(oldName, newName);
                  if (saved) {
                    await refreshSpecOptions(editingRoom);
                    await loadRooms();
                    if (roomForm.door_style === oldName) {
                      setRoomForm((prev) => ({ ...prev, door_style: saved }));
                    }
                  }
                  return saved;
                }}
              ...
              <CreatableSpecSelect
                ...
                onRenameOption={async (oldName, newName) => {
                  const saved = await renameBaseMoldingOption(oldName, newName);
                  if (saved) {
                    await refreshSpecOptions(editingRoom);
                    await loadRooms();
                    if (roomForm.base_molding === oldName) {
                      setRoomForm((prev) => ({ ...prev, base_molding: saved }));
                    }
                  }
                  return saved;
                }}
