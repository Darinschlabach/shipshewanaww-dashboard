...
import type { PricingCabinetType } from "@/lib/pricing-catalogue";
...
interface CabinetTypesTabProps {
  cabinets: PricingCabinetType[];
  loading: boolean;
...
export default function CabinetTypesTab({
  cabinets,
  loading,
  onReload,
  onSave,
  onDelete,
}: CabinetTypesTabProps) {
  ...
  const categories = useMemo(
    () => [...new Set(cabinets.map((c) => c.category))].sort(),
    [cabinets]
  );
  ...
  }, [cabinets, search, categoryFilter]);
  ...
  function openCreate() {
    ...
    setForm({
      name: "",
      category: categories[0] ?? "Base Cabinets",
      base_price: "",
      sq_ft_price: "",
    });
    setShowModal(true);
  ...
  function openEdit(item: PricingCabinetType) {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this cabinet type?")) return;
    setMenuId(null);
    await onDelete(id);
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Cabinet Types</h2>
          <p className="mt-0.5 text-sm text-gray-500">
            Set base prices and square foot prices for each cabinet type.
          </p>
        </div>
      ...
      {loading ? (
        <p className="px-5 py-12 text-sm text-gray-500">Loading cabinet types…</p>
      ) : (
        <>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  <th className={headClass}>Cabinet Type</th>
                  <th className={headClass}>Category</th>
                  <th className={headClass}>
                    <span className="inline-flex items-center gap-1">
                      Base Price
                      <span title="Fixed price for this cabinet type">
              ...
              <tbody>
                ...
                {pageItems.length === 0 && (
                  <tr>
                    ...
                    >
                      No cabinet types match your search.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 px-5 py-3 text-sm text-gray-600">
            <p>
              {filtered.length === 0
                ? "No cabinet types"
                : `Showing ${pageStart + 1} to ${Math.min(pageStart + pageSize, filtered.length)} of ${filtered.length} cabinet type${filtered.length !== 1 ? "s" : ""}`}
            </p>
      ...
      {showModal && (
        <Modal
          title={editingId ? "Edit cabinet type" : "Add cabinet type"}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                list="cabinet-categories"
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              />
              <datalist id="cabinet-categories">
                {categories.map((cat) => (
                  <option key={cat} value={cat} />
                ))}
              </datalist>
